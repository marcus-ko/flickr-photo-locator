...
$(function() {
    ...
    // Hide the map when the page loads
    $('.modal').hide();
    $( "#query" ).autocomplete({
      source: cities
    });
    ...
    // $('#query').autocomplete({
    // serviceUrl: '/autocomplete/availableTags',
    // onSelect: function (suggestion) {
    //     alert('You selected: ' + suggestion.value + ', ' + suggestion.data);
    // }
    // });

    // When submit button is clicked, show the map, hide the form
    $('#search-term').submit(function(event) {
        event.preventDefault();
        var searchTerm = $('#query').val();
        getRequest(searchTerm);
        $('.modal').show();
        $('#search-results').delay(7000).fadeIn();
        $('#map_canvas').show();
        $('.form').hide();
        $('#map').delay(5000).fadeIn();
    })
    ...
    // Combine url and search term to return results 
    function getRequest(searchTerm) {
        var params = {
            method: "flickr.photos.search",
            api_key: "e0812ea48c0281dd54ffa944e052a686",
            tags: searchTerm,
            has_geo: '1',
            radius: '5',
            radius_units: 'mi',
        ...
        $.getJSON(url, params, function(data) {
            ...
            // photoResults data is good
            var photoResults = data.photos.photo;
            ...
            for (i = 0; i < photoResults.length; i++) {
                // Get geo data
                (function(n) {
                    var photo = photoResults[n];

                    $.getJSON('https://api.flickr.com/services/rest/?method=flickr.photos.geo.getLocation&api_key=e0812ea48c0281dd54ffa944e052a686&photo_id=' + photo.id + '&format=json&nojsoncallback=1',
                        function(geoData) {
                            photo.lat = geoData.photo.location.latitude;
                            photo.long = geoData.photo.location.longitude;
                            photo.neighbourhood = geoData.photo.location.neighbourhood._content;
            ...
            console.log('processedPhotos', processedPhotos);
           
            setTimeout(function() {
                var locs = processedPhotos.map(function(item) {
                        // console.log('processing');
                        // '<img src=' + 'http://farm' + value.farm + '.staticflickr.com/' + value.server + '/' +
                        //                 value.id + '_' + value.secret + '_t.jpg' + '/>'
                        // console.log(item);
                        return {
                            lat: +item.lat, // latitude
                            long: +item.long, // longitude
                            id: item.id, // user id
                            farm: item.farm, // farm
                            server: item.server, // server
                            secret: item.secret, // flickr secret
                            title: item.title,
                            owner: item.owner,
                            neighbourhood: item.neighbourhood._content // name of neighborhood
                        };
                    })
                    // console.log('waiting', locs);
                ...
                showResults(processedPhotos); // contains the essential results
                $('.reset').delay(7000).fadeIn();
            }, 5000);
    ...
    function showResults(results) {
        // $( ".fa.fa-arrow-down" ).delay(2000).fadeIn().animate({ "top": "+=20px" }, 1000 ).fadeOut();
        // $('.newBackground').show();
        // $('main').hide();

        $(".fa.fa-arrow-down").fadeIn();
        $(".scroll").fadeIn();
        for (animCount = 0; animCount < 5; animCount++) {
            $(".fa.fa-arrow-down").animate({ "top": "+=10px" });
            $(".fa.fa-arrow-down").animate({ "top": "-=10px" });
            animCount += 1;
        }


        $(".fa.fa-arrow-down").delay().fadeOut();
        $(".scroll").delay(2000).fadeOut();


        var html = "";
        $.each(results, function(index, value) {
            html += "<div><h2>" + value.title + "</h2><br>" + "<a href='http://flickr.com/photos/" + 
            value.owner + "/" + value.id + "'" + " target='_blank'>" +
            "<img src=" + 'http://farm' + value.farm + '.staticflickr.com/' + value.server + '/' +
            value.id + '_' + value.secret + '_z.jpg' +  " /></a></div>";
        });
...
function initialize(locs) {
    mapboxgl.accessToken = 'pk.eyJ1IjoibWFyY3VzazcxIiwiYSI6ImNrYjdjdmp1NDA0aXYycXNibXh2cGt4dHUifQ.TS-QfwCfE24AHeqeuSEAwQ';
    ...
    for (var maparray = 0; maparray < locs.length; maparray ++) {
      markers.push([locs[maparray].long, locs[maparray].lat]);
    }
    ...
    //         // markers.push([locs[m].neighbourhood, locs[m].lat, locs[m].long]);
    //         markers.push([locs[m].lat, locs[m].long]);
    //         infoWindowContent.push(['<img src=' + 'http://farm' + locs[m].farm + '.staticflickr.com/' + locs[m].server + '/' +
    //             locs[m].id + '_' + locs[m].secret + '_m.jpg' + '/>'
    //         ]);
    //     }
    // }
    ...
    if (typeof locs !== "undefined") {
        for (var m = 0; m < locs.length; m++) {
            ...
            .setLngLat([locs[m].long, locs[m].lat])
            .addTo(map);
            markers.push([locs[m].long, locs[m].lat]);
            ...
            var geojson = {
                ...
                geometry: {
                    type: 'Point',
                    coordinates: [locs[m].long, locs[m].lat],
                    zoom: 6
                },
                properties: {
                    title: locs[m].title,
            ...
            geojson.features.forEach(function(marker) {
                ...
                .setPopup(new mapboxgl.Popup({ offset: 25 }) // add popups
                  .setHTML('<p><a href="http://flickr.com/photos/' + marker.properties.owner + '/' + marker.properties.id + '"' + 'target="_blank">' + marker.properties.title + '</a></p>'
                //   '<a href="http://flickr.com/photos/" + value.owner + "/" + value.id + "'" + " target='_blank'>'
                  ))
                .addTo(map);
    ...
    //   add.locations.forEach((locs) => {
    //     console.log(location.long + " " + location.lat);
    //     // add marker to map
    //     var marker = new mapboxgl.Marker(el)
    //         .setLngLat([locs.long, locs.lat])
    //         .setPopup(popup)
    //         .addTo(this.map);
    ...
    //     for (mCount = 0; mCount < markers.length; mCount++) {
    //         console.log('markers:', markers[mCount]);
    //         var markerPosition = new google.maps.LatLng(markers[mCount][0], markers[mCount][1]);

    //         bounds.extend(markerPosition);
    //         marker = new google.maps.Marker({
    //             position: markerPosition,
    //             map: map
    //                 // title: markers[mCount][0]
    //         });


    //         // Allow each marker to have an info window    
    //         google.maps.event.addListener(marker, 'click', (function(marker, mCount) {
    //             return function() {
    //                 infoWindow.setContent(infoWindowContent[mCount][0]);
    ...
    // }
    // console.log('position1', markerPosition);
...
// Autocomplete for City and State
...
function autocomplete(inp, arr) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            b.innerHTML = "<p>" + arr[i].substr(0, val.length) + "</p>";
  ...
  var cities = ['Abbeville, Louisiana',
  'Aberdeen, Maryland',
  ...
  autocomplete(document.getElementById("query"), cities);